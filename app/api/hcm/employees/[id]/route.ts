...
export async function GET(_req: Request, context: { params: { id: string } }) {
  // CURSOR: API call to GET /api/v1/hcm/employees/{id}
  const base = process.env.API_BASE_URL
  if (!base) {
    const item = fallbackEmployees.find(e => e.id === context.params.id) || null
    return NextResponse.json({ data: item })
  }
  try {
    const url = new URL(`/api/v1/hcm/employees/${encodeURIComponent(context.params.id)}`, base)
...
export async function PATCH(req: Request, context: { params: { id: string } }) {
  // CURSOR: API call to PATCH /api/v1/hcm/employees/{id}
  const base = process.env.API_BASE_URL
  const body = await req.json().catch(() => ({}))
  if (!base) {
    const existing = fallbackEmployees.find(e => e.id === context.params.id) || null
