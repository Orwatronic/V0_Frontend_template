...
const fallback: Account[] = [
  { id: 'AC-1001', name: 'Contoso Ltd', type: 'enterprise', status: 'active', country: 'US' },
  { id: 'AC-1002', name: 'Northwind', type: 'sme', status: 'pending', country: 'UK' },
  { id: 'AC-1003', name: 'Fabrikam', type: 'startup', status: 'inactive', country: 'DE' },
]
...
export async function GET(req: Request) {
  ...
  const sortBy = (incoming.searchParams.get('sortBy') || 'name') as keyof Account
  ...
  if (!base) {
    let data = [...fallback]
    if (q) {
      data = data.filter((r) => r.id.toLowerCase().includes(q) || r.name.toLowerCase().includes(q) || r.country.toLowerCase().includes(q))
    }
    data.sort((a, b) => {
  ...
  const url = new URL('/api/v1/crm/accounts', base)
