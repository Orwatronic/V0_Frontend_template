...
// GET /api/crm/customers/:id/invoices
...
export async function GET(req: Request, ctx: { params: { id: string } }) {
  ...
  if (!base) {
    return NextResponse.json({
      data: [
        { id: 'inv1', number: 'INV-1001', status: 'open', dueDate: '2025-09-01', total: 12450, currency: 'USD' },
        { id: 'inv2', number: 'INV-1002', status: 'overdue', dueDate: '2025-08-01', total: 5800, currency: 'USD' },
      ],
    })
  }

  const incoming = new URL(req.url)
  const url = new URL(`/api/v1/crm/customers/${encodeURIComponent(id)}/invoices`, base)
