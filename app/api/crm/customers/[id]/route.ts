...
// GET /api/crm/customers/:id -> forwards to backend
...
export async function GET(req: Request, ctx: { params: { id: string } }) {
  ...
  // development fallback if API_BASE_URL is not configured
  if (!base) {
    return NextResponse.json({
      data: {
        ...
        primaryContact: { id: 'pc1', name: 'Alex Doe', email: 'alex@example.com', phone: '+1 555 0100' },
        billingAddress: '1 Main St, City',
        shippingAddress: 'Warehouse Rd, City',
      },
    })
  }

  const incoming = new URL(req.url)
  const url = new URL(`/api/v1/crm/customers/${encodeURIComponent(id)}`, base)
