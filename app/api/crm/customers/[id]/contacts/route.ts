...
export async function GET(req: Request, ctx: { params: { id: string } }) {
  ...
  if (!base) {
    return NextResponse.json({
      data: [
        { id: 'c1', name: 'Fatima Noor', role: 'CFO', email: 'fatima@sample.io', phone: '+971 55 123 4567' },
        { id: 'c2', name: 'Jonas Berg', role: 'Head of IT', email: 'jonas@sample.io', phone: '+47 41 123 456' },
      ],
    })
  }

  const incoming = new URL(req.url)
  const url = new URL(`/api/v1/crm/customers/${encodeURIComponent(id)}/contacts`, base)
