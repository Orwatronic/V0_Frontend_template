    ...
    <section aria-label={t("crm.customer360.title")} className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">{t("crm.customer360.subtitle")}</p>
        </div>
        {overview?.code ? (
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="overview" className="space-y-4">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                <div>
                  <span className="font-medium">{t("crm.customer360.overview.segment")}:</span>{" "}
                  {overview?.segment ?? "—"}
                </div>
                <div>
                  <span className="font-medium">{t("crm.customer360.overview.industry")}:</span>{" "}
                  {overview?.industry ?? "—"}
                </div>
              </CardContent>
            </Card>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">{t("crm.customer360.metrics.arr")}</CardTitle>
              </CardHeader>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">{t("crm.customer360.metrics.openOpps")}</CardTitle>
              </CardHeader>
              <CardContent className="text-2xl font-semibold">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">{t("crm.customer360.metrics.arAgingCurrent")}</CardTitle>
              </CardHeader>
              <CardContent className="text-2xl font-semibold">{aging.current?.toLocaleString?.() ?? "—"}</CardContent>
            </Card>
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">{t("crm.customer360.metrics.arAging90")}</CardTitle>
              </CardHeader>
              <CardContent className="text-2xl font-semibold">{aging.d90?.toLocaleString?.() ?? "—"}</CardContent>
            </Card>
          </div>
        </TabsContent>
        ...
        <TabsContent value="contacts">
          {contacts?.length ? (
            <Card>
              <CardHeader>
                <CardTitle>{t("crm.customer360.contacts.title")}</CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
            ...
            <p className="text-sm text-muted-foreground">
              {loading ? t("crm.customer360.loading") : t("crm.customer360.contacts.empty")}
            </p>
          )}
        </TabsContent>
        ...
        <TabsContent value="opportunities">
          {opps?.length ? (
            <Card>
              <CardHeader>
                <CardTitle>{t("crm.customer360.opps.title")}</CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                    ...
                    {opps.map((o) => (
                      <TableRow key={o.id}>
                        ...
                        <TableCell className="text-muted-foreground">
                          {o.closeDate
                            ? dateFormatter
                              ? dateFormatter.format(new Date(o.closeDate))
                              : o.closeDate
                            : "—"}
                        </TableCell>
                      </TableRow>
            ...
            <p className="text-sm text-muted-foreground">
              {loading ? t("crm.customer360.loading") : t("crm.customer360.opps.empty")}
            </p>
          )}
        </TabsContent>
        ...
        <TabsContent value="activities">
          {activities?.length ? (
            <Card>
              <CardHeader>
                <CardTitle>{t("crm.customer360.activities.title")}</CardTitle>
              </CardHeader>
              <CardContent>
                    ...
                    <li key={a.id} className="rounded-md border p-3">
                      <div className="flex items-center justify-between">
                        <span className="font-medium">
                          {a.type.toUpperCase()} • {a.subject ?? t("crm.customer360.activities.noSubject")}
                        </span>
            ...
            <p className="text-sm text-muted-foreground">
              {loading ? t("crm.customer360.loading") : t("crm.customer360.activities.empty")}
            </p>
          )}
        </TabsContent>
        ...
        <TabsContent value="invoices">
          {invoices?.length ? (
            <Card>
              <CardHeader>
                <CardTitle>{t("crm.customer360.invoices.title")}</CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
            ...
            <p className="text-sm text-muted-foreground">
              {loading ? t("crm.customer360.loading") : t("crm.customer360.invoices.empty")}
            </p>
          )}
        </TabsContent>
