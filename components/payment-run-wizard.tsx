...
import { ArrowRight, ArrowLeft, Banknote, Calendar, CheckCircle, XCircle, Loader2 } from 'lucide-react'
...
const mockPayableInvoices = [
  { id: 'INV-002', vendor: 'Office Solutions LLC', dueDate: '2024-08-19', amount: 750.50, discount: 15.01 },
  { id: 'INV-003', vendor: 'Cloud Services Co.', dueDate: '2024-08-21', amount: 3500.00, discount: 70.00 },
...
export const PaymentRunWizard = ({ isOpen, onClose }: PaymentRunWizardProps) => {
            ...
            <div className="border rounded-md max-h-[400px] overflow-y-auto">
              <Table>
                <TableHeader className="sticky top-0 bg-background">
                  <TableRow>
                    <TableHead className="w-[50px]"><Checkbox checked={Object.values(selectedInvoices).every(Boolean)} onCheckedChange={(checked) => setSelectedInvoices(payableInvoices.reduce((acc, inv) => ({ ...acc, [inv.id]: !!checked }), {}))} /></TableHead>
                ...
                <TableBody>
                  {payableInvoices.map(inv => (
                    <TableRow key={inv.id}>
                      <TableCell><Checkbox checked={selectedInvoices[inv.id]} onCheckedChange={(checked) => setSelectedInvoices(prev => ({ ...prev, [inv.id]: !!checked }))} /></TableCell>
  ...
  const getDialogTitle = () => {
    switch (step) {
      case 1: return t('ppw.step1.title');
      case 2: return t('ppw.step2.title');
      case 3: return t('ppw.step3.title');
      case 4: return t('ppw.step4.title');
      default: return t('ppw.title');
    }
  };
      ...
      <DialogContent className="max-w-3xl">
        ...
        <DialogFooter>
          ...
          {step === 4 ? (
            ...
            <Button variant="ghost" onClick={handleClose} disabled={isLoading}>{t('common.cancel')}</Button>
          )}
        </DialogFooter>
      </DialogContent>
    </Dialog>
